% Function to cross-validate and test the slle algorithm.
% By Fei Xiong
%    ECE Dept.,
...
function [validation_fold,test_fold,SLLEStruct,error_rate_sLLE] ...
    = test_sLLEv1(name_data, method, mode , varargin)
num_train_fold=[];
if size(varargin)>0
...
%                 display(['================  NN = ' num2str(NN) ' ================'])
                [LD] = sllev1(X', class_train, NN, alpha);
                sLLE_LD{j,1} = LD;
        ...
        display(['================ test ================'])
        save([name_data '_' num2str(num_samp) 'sample' '_' num2str(num_fold) 'fold_sLLEv1_experiment.mat'],...
