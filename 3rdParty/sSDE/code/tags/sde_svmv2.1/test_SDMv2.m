% Function that only test the classification accuracy for SDM manifold. The dimensionality of the
...
function [validation_fold,test_fold,SDMStruct,error_rate_SDM] = test_SDMv2(name_data,varargin)
num_train_fold=[];
if size(varargin)>0
...
for k=1:size(exp_pair,1);
    ...
    for i=1:num_fold
        ...
        load([name_data '_' num2str(num_samp) 'sample' '_' ...
            num2str(num_fold) 'fold_SDESVMKMF_' num2str(Nl) '_' 'testfold_LLW_S_experiment.mat'],...
            'SDESVMKMFStruct');
        %% SDM %%
        ...
        error_temp = error(d,:);
        [~, jj]=min(error_temp);
        SDMStruct(k,i).dim=dim_valid{d, jj}; SDMStruct(k,i).Z=Z_valid{d, jj};
        SDMStruct(k,i).W=W_valid{d, jj}; SDMStruct(k,i).theta=theta_valid{d, jj};
        SDMStruct(k,i).D=D_valid{d, jj}; SDMStruct(k,i).mu=mu_valid{d, jj};
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %%%%%%%%%%%%%%%%%%%%%   test               %%%%%%%%%%%%%%%%%%%
        W= SDMStruct(k,i).W;
        Z= SDMStruct(k,i).Z;
