% Function to cross-validate and test the SDE.
% By Fei Xiong
%    ECE Dept.,
...
%   
% OUTPUT
%       SDEStruct: the structure where all the training parameter and
...
function [validation_fold,test_fold,SDEStruct,error_rate_SDE] = test_SDE(name_data,dim_thresh,method, mode , varargin)
num_test_fold=[];
if size(varargin)>0
...
for j=1:4
    ...
    for k=1:size(exp_pair,1);
        ...
        parfor i=1:num_fold
            ...
            t1 = cputime;
            [LD, ~] = SDEcvx(X, eps, NN); % SDE embedding
            t_valid3{j,i} = cputime;
            t_valid3{j,i} = t_valid3{j,i} -t1;
            K_valid3{j,i} = LD*LD';
        end
